import React, { useState } from 'react';
import {
  Trophy,
  Target,
  GraduationCap,
  Flame,
  Calendar,
  Edit2,
  Check,
  X,
  Plus,
  Trash2,
  Compass,
  Shield,
  Zap,
  Heart,
  FileText,
  RefreshCw,
} from 'lucide-react';
import { CoachPhilosophy } from '../../types';
import { SeasonGoal } from '../../types/planning';
import { diffPhilosophy } from '../../services/philosophyService';

interface SeasonGoalManagerProps {
  season: SeasonGoal;
  onSave: (updated: SeasonGoal) => void;
  isPhilosophyComplete?: boolean;
  missingCriticalFields?: string[];
  currentPhilosophy?: CoachPhilosophy | null;
  onNavigateToPhilosophy?: () => void;
  onUpdateSnapshot?: () => void;
}

export const SeasonGoalManager: React.FC<SeasonGoalManagerProps> = ({
  season,
  onSave,
  isPhilosophyComplete = true,
  missingCriticalFields = [],
  currentPhilosophy,
  onNavigateToPhilosophy,
  onUpdateSnapshot,
}) => {
  const [isEditing, setIsEditing] = useState(false);
  const [formData, setFormData] = useState<SeasonGoal>({ ...season });
  const [newSportGoal, setNewSportGoal] = useState('');
  const [newFormGoal, setNewFormGoal] = useState('');

  // Use snapshot if available, fallback to current philosophy
  const displayPhilosophy: Partial<CoachPhilosophy> =
    season.philosophySnapshot || currentPhilosophy || {};

  const hasDiffs =
    isPhilosophyComplete &&
    season.philosophySnapshot &&
    currentPhilosophy &&
    diffPhilosophy(currentPhilosophy, season.philosophySnapshot).length > 0;

  const handleSave = () => {
    if (!isPhilosophyComplete) return;
    onSave(formData);
    setIsEditing(false);
  };

  const handleCancel = () => {
    setFormData({ ...season });
    setIsEditing(false);
  };

  const addSportGoal = () => {
    if (newSportGoal.trim()) {
      setFormData({
        ...formData,
        objetivosDeportivos: [...formData.objetivosDeportivos, newSportGoal.trim()],
      });
      setNewSportGoal('');
    }
  };

  const removeSportGoal = (index: number) => {
    setFormData({
      ...formData,
      objetivosDeportivos: formData.objetivosDeportivos.filter((_, i) => i !== index),
    });
  };

  const addFormGoal = () => {
    if (newFormGoal.trim()) {
      setFormData({
        ...formData,
        objetivosFormativos: [...formData.objetivosFormativos, newFormGoal.trim()],
      });
      setNewFormGoal('');
    }
  };

  const removeFormGoal = (index: number) => {
    setFormData({
      ...formData,
      objetivosFormativos: formData.objetivosFormativos.filter((_, i) => i !== index),
    });
  };

  return (
    <div className="space-y-6">
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-orange-600 via-amber-600 to-orange-700 rounded-2xl p-6 text-white shadow-md flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-orange-100 text-sm font-medium mb-1">
            <Trophy className="w-4 h-4" />
            <span>Objetivos Globales de la Temporada</span>
          </div>
          <h2 className="text-2xl font-bold tracking-tight">
            Temporada {formData.temporada} • {formData.categoria}
          </h2>
          <p className="text-orange-50 text-sm mt-1 max-w-2xl opacity-90">
            Marco estratégico rector para todos los mesociclos, microciclos y sesiones de entrenamiento.
          </p>
        </div>

        <div className="flex items-center gap-3">
          {isEditing ? (
            <>
              <button
                type="button"
                onClick={handleCancel}
                className="px-4 py-2 bg-white/20 hover:bg-white/30 text-white rounded-lg text-sm font-medium transition flex items-center gap-1.5 cursor-pointer"
              >
                <X className="w-4 h-4" />
                <span>Cancelar</span>
              </button>
              <button
                type="button"
                onClick={handleSave}
                disabled={!isPhilosophyComplete}
                className={`px-4 py-2 rounded-lg text-sm font-semibold shadow-sm transition flex items-center gap-1.5 cursor-pointer ${
                  isPhilosophyComplete
                    ? 'bg-white text-orange-700 hover:bg-orange-50'
                    : 'bg-slate-200 text-slate-400 cursor-not-allowed'
                }`}
              >
                <Check className="w-4 h-4" />
                <span>Guardar Cambios</span>
              </button>
            </>
          ) : (
            <button
              type="button"
              onClick={() => {
                if (!isPhilosophyComplete) {
                  onNavigateToPhilosophy?.();
                } else {
                  setIsEditing(true);
                }
              }}
              className="px-4 py-2 bg-white text-orange-700 hover:bg-orange-50 rounded-lg text-sm font-semibold shadow-sm transition flex items-center gap-1.5 cursor-pointer"
            >
              <Edit2 className="w-4 h-4" />
              <span>{isPhilosophyComplete ? 'Editar Objetivos' : 'Completar Filosofía'}</span>
            </button>
          )}
        </div>
      </div>

      {/* Widget: Filosofía de Entrenador Vinculada a la Temporada */}
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-orange-100 text-orange-700 rounded-xl">
              <Compass className="w-5 h-5" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-bold text-slate-900">
                  Filosofía de Entrenador Vinculada
                </h3>
                {season.philosophySnapshot ? (
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 font-bold">
                    Snapshot Activo
                  </span>
                ) : (
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 font-bold">
                    Sin Snapshot Fijo
                  </span>
                )}
              </div>
              <p className="text-xs text-slate-500">
                Identidad táctica y pedagógica que rige todas las sugerencias y evaluaciones de la IA.
              </p>
            </div>
          </div>

          <div className="flex items-center gap-2">
            {hasDiffs && onUpdateSnapshot && (
              <button
                type="button"
                onClick={onUpdateSnapshot}
                className="px-3 py-1.5 bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200 rounded-lg text-xs font-semibold transition flex items-center gap-1.5 cursor-pointer"
              >
                <RefreshCw className="w-3.5 h-3.5" />
                <span>Sincronizar Snapshot</span>
              </button>
            )}
            {onNavigateToPhilosophy && (
              <button
                type="button"
                onClick={onNavigateToPhilosophy}
                className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-xs font-semibold transition flex items-center gap-1.5 cursor-pointer"
              >
                <Compass className="w-3.5 h-3.5 text-orange-600" />
                <span>Ir a Filosofía de Entrenador</span>
              </button>
            )}
          </div>
        </div>

        {/* 7 Philosophy Fields Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5 text-xs">
          {/* 1. Estilo de Juego */}
          <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200/80 space-y-1">
            <div className="flex items-center gap-1.5 font-bold text-slate-800">
              <Flame className="w-3.5 h-3.5 text-amber-600" />
              <span>Estilo de Juego</span>
            </div>
            <p className="text-slate-600 font-medium line-clamp-3">
              {displayPhilosophy.playStyle || <span className="text-amber-600 italic">Pendiente</span>}
            </p>
          </div>

          {/* 2. Foco Ofensivo */}
          <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200/80 space-y-1">
            <div className="flex items-center gap-1.5 font-bold text-slate-800">
              <Zap className="w-3.5 h-3.5 text-blue-600" />
              <span>Foco Ofensivo</span>
            </div>
            <p className="text-slate-600 font-medium line-clamp-3">
              {displayPhilosophy.offensiveFocus || <span className="text-amber-600 italic">Pendiente</span>}
            </p>
          </div>

          {/* 3. Foco Defensivo */}
          <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200/80 space-y-1">
            <div className="flex items-center gap-1.5 font-bold text-slate-800">
              <Shield className="w-3.5 h-3.5 text-emerald-600" />
              <span>Foco Defensivo</span>
            </div>
            <p className="text-slate-600 font-medium line-clamp-3">
              {displayPhilosophy.defensiveFocus || <span className="text-amber-600 italic">Pendiente</span>}
            </p>
          </div>

          {/* 4. Objetivos de Entrenamiento */}
          <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200/80 space-y-1">
            <div className="flex items-center gap-1.5 font-bold text-slate-800">
              <Target className="w-3.5 h-3.5 text-purple-600" />
              <span>Objetivos de Entrenamiento</span>
            </div>
            <p className="text-slate-600 font-medium line-clamp-3">
              {displayPhilosophy.trainingGoals || <span className="text-slate-400 italic">Opcional / No definido</span>}
            </p>
          </div>

          {/* 5. Objetivos de Partido */}
          <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200/80 space-y-1">
            <div className="flex items-center gap-1.5 font-bold text-slate-800">
              <Trophy className="w-4 h-4 text-orange-600" />
              <span>Objetivos de Partido</span>
            </div>
            <p className="text-slate-600 font-medium line-clamp-3">
              {displayPhilosophy.matchGoals || <span className="text-slate-400 italic">Opcional / No definido</span>}
            </p>
          </div>

          {/* 6. Valores Fundamentales */}
          <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200/80 space-y-1">
            <div className="flex items-center gap-1.5 font-bold text-slate-800">
              <Heart className="w-3.5 h-3.5 text-rose-600" />
              <span>Valores Fundamentales</span>
            </div>
            <p className="text-slate-600 font-medium line-clamp-3">
              {displayPhilosophy.coreValues || <span className="text-amber-600 italic">Pendiente</span>}
            </p>
          </div>

          {/* 7. Notas Adicionales */}
          <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200/80 space-y-1 md:col-span-2 lg:col-span-3">
            <div className="flex items-center gap-1.5 font-bold text-slate-800">
              <FileText className="w-3.5 h-3.5 text-slate-600" />
              <span>Directrices y Modelo Formativo (Notas del Entrenador)</span>
            </div>
            <p className="text-slate-600 font-medium line-clamp-3">
              {displayPhilosophy.additionalNotes || <span className="text-slate-400 italic">Sin directrices adicionales</span>}
            </p>
          </div>
        </div>
      </div>

      {isEditing ? (
        // EDIT MODE
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">
                Temporada (p. ej. 2025-2026)
              </label>
              <input
                type="text"
                value={formData.temporada}
                onChange={(e) => setFormData({ ...formData, temporada: e.target.value })}
                className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-lg text-sm text-slate-800 focus:bg-white focus:border-orange-500 focus:outline-none"
              />
            </div>
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">
                Categoría
              </label>
              <input
                type="text"
                value={formData.categoria}
                onChange={(e) => setFormData({ ...formData, categoria: e.target.value })}
                className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-lg text-sm text-slate-800 focus:bg-white focus:border-orange-500 focus:outline-none"
              />
            </div>
          </div>

          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">
              Objetivo Principal de la Temporada
            </label>
            <textarea
              rows={2}
              value={formData.objetivoPrincipal}
              onChange={(e) => setFormData({ ...formData, objetivoPrincipal: e.target.value })}
              className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-lg text-sm text-slate-800 focus:bg-white focus:border-orange-500 focus:outline-none"
              placeholder="p. ej. Ascenso a Primera División Nacional y consolidación del modelo de juego colectivo."
            />
          </div>

          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">
              Estilo de Juego Propuesto
            </label>
            <textarea
              rows={2}
              value={formData.estiloDeJuego}
              onChange={(e) => setFormData({ ...formData, estiloDeJuego: e.target.value })}
              className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-lg text-sm text-slate-800 focus:bg-white focus:border-orange-500 focus:outline-none"
              placeholder="p. ej. Transición rápida tras rebote, 5 abiertos con penetrar y doblar, presión a 3/4."
            />
          </div>

          {/* Objetivos Deportivos */}
          <div className="space-y-3 pt-2 border-t border-slate-100">
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-700">
              Objetivos Deportivos / Competitivos
            </label>
            <div className="flex gap-2">
              <input
                type="text"
                value={newSportGoal}
                onChange={(e) => setNewSportGoal(e.target.value)}
                placeholder="Añadir objetivo deportivo..."
                className="flex-1 px-3.5 py-2 bg-slate-50 border border-slate-300 rounded-lg text-sm text-slate-800 focus:bg-white focus:border-orange-500 focus:outline-none"
                onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), addSportGoal())}
              />
              <button
                type="button"
                onClick={addSportGoal}
                className="px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-lg text-sm font-semibold transition flex items-center gap-1 cursor-pointer"
              >
                <Plus className="w-4 h-4" />
                <span>Añadir</span>
              </button>
            </div>
            <div className="space-y-2">
              {formData.objetivosDeportivos.map((goal, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-800"
                >
                  <span className="flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-emerald-500" />
                    {goal}
                  </span>
                  <button
                    type="button"
                    onClick={() => removeSportGoal(idx)}
                    className="text-slate-400 hover:text-red-600 transition p-1 cursor-pointer"
                  >
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
              ))}
            </div>
          </div>

          {/* Objetivos Formativos */}
          <div className="space-y-3 pt-2 border-t border-slate-100">
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-700">
              Objetivos Formativos / Desarrollo
            </label>
            <div className="flex gap-2">
              <input
                type="text"
                value={newFormGoal}
                onChange={(e) => setNewFormGoal(e.target.value)}
                placeholder="Añadir objetivo formativo..."
                className="flex-1 px-3.5 py-2 bg-slate-50 border border-slate-300 rounded-lg text-sm text-slate-800 focus:bg-white focus:border-orange-500 focus:outline-none"
                onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), addFormGoal())}
              />
              <button
                type="button"
                onClick={addFormGoal}
                className="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg text-sm font-semibold transition flex items-center gap-1 cursor-pointer"
              >
                <Plus className="w-4 h-4" />
                <span>Añadir</span>
              </button>
            </div>
            <div className="space-y-2">
              {formData.objetivosFormativos.map((goal, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-800"
                >
                  <span className="flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-purple-500" />
                    {goal}
                  </span>
                  <button
                    type="button"
                    onClick={() => removeFormGoal(idx)}
                    className="text-slate-400 hover:text-red-600 transition p-1 cursor-pointer"
                  >
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
              ))}
            </div>
          </div>

          {/* Dates */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-slate-100">
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">
                Fecha Inicio
              </label>
              <input
                type="date"
                value={formData.fechaInicio}
                onChange={(e) => setFormData({ ...formData, fechaInicio: e.target.value })}
                className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-lg text-sm text-slate-800 focus:bg-white focus:border-orange-500 focus:outline-none"
              />
            </div>
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">
                Fecha Fin
              </label>
              <input
                type="date"
                value={formData.fechaFin}
                onChange={(e) => setFormData({ ...formData, fechaFin: e.target.value })}
                className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-lg text-sm text-slate-800 focus:bg-white focus:border-orange-500 focus:outline-none"
              />
            </div>
          </div>
        </div>
      ) : (
        // VIEW MODE
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Main Goal Card */}
          <div className="md:col-span-2 bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:border-slate-300 transition">
            <div className="flex items-center gap-2.5 mb-2.5">
              <div className="w-8 h-8 rounded-lg bg-orange-100 text-orange-600 flex items-center justify-center font-bold">
                <Target className="w-4 h-4" />
              </div>
              <h3 className="font-bold text-slate-900 text-base">Objetivo Principal de Temporada</h3>
            </div>
            <p className="text-slate-700 text-base font-medium leading-relaxed pl-10">
              &quot;{formData.objetivoPrincipal}&quot;
            </p>
          </div>

          {/* Style of Play Card */}
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:border-slate-300 transition">
            <div className="flex items-center gap-2.5 mb-3">
              <div className="w-8 h-8 rounded-lg bg-amber-100 text-amber-600 flex items-center justify-center">
                <Flame className="w-4 h-4" />
              </div>
              <h3 className="font-bold text-slate-900 text-base">Estilo de Juego e Identidad</h3>
            </div>
            <p className="text-slate-600 text-sm leading-relaxed pl-10">
              {formData.estiloDeJuego || 'No definido aún.'}
            </p>
          </div>

          {/* Dates & Duration Card */}
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:border-slate-300 transition">
            <div className="flex items-center gap-2.5 mb-3">
              <div className="w-8 h-8 rounded-lg bg-blue-100 text-blue-600 flex items-center justify-center">
                <Calendar className="w-4 h-4" />
              </div>
              <h3 className="font-bold text-slate-900 text-base">Periodo y Calendario</h3>
            </div>
            <div className="pl-10 space-y-1.5 text-sm">
              <div className="flex items-center justify-between text-slate-600">
                <span>Inicio de temporada:</span>
                <span className="font-semibold text-slate-800">{formData.fechaInicio || 'Sep 2025'}</span>
              </div>
              <div className="flex items-center justify-between text-slate-600">
                <span>Cierre previsto:</span>
                <span className="font-semibold text-slate-800">{formData.fechaFin || 'Jun 2026'}</span>
              </div>
            </div>
          </div>

          {/* Sporting Goals */}
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:border-slate-300 transition">
            <div className="flex items-center gap-2.5 mb-4">
              <div className="w-8 h-8 rounded-lg bg-emerald-100 text-emerald-600 flex items-center justify-center">
                <Trophy className="w-4 h-4" />
              </div>
              <h3 className="font-bold text-slate-900 text-base">Objetivos Deportivos</h3>
            </div>
            <ul className="space-y-2.5 pl-2">
              {formData.objetivosDeportivos.map((goal, idx) => (
                <li key={idx} className="flex items-start gap-2.5 text-sm text-slate-700">
                  <span className="w-2 h-2 rounded-full bg-emerald-500 mt-1.5 shrink-0" />
                  <span>{goal}</span>
                </li>
              ))}
            </ul>
          </div>

          {/* Formative Goals */}
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:border-slate-300 transition">
            <div className="flex items-center gap-2.5 mb-4">
              <div className="w-8 h-8 rounded-lg bg-purple-100 text-purple-600 flex items-center justify-center">
                <GraduationCap className="w-4 h-4" />
              </div>
              <h3 className="font-bold text-slate-900 text-base">Objetivos Formativos</h3>
            </div>
            <ul className="space-y-2.5 pl-2">
              {formData.objetivosFormativos.map((goal, idx) => (
                <li key={idx} className="flex items-start gap-2.5 text-sm text-slate-700">
                  <span className="w-2 h-2 rounded-full bg-purple-500 mt-1.5 shrink-0" />
                  <span>{goal}</span>
                </li>
              ))}
            </ul>
          </div>
        </div>
      )}
    </div>
  );
};
