...
import { downloadTacticalPlayVideo } from '../utils/tacticalVideoExport';
...
export const WhiteboardView: React.FC<WhiteboardViewProps> = ({
  ...
  const handleExportMp4Video = async (customPlay?: SavedPlay) => {
    const targetPlayTitle = customPlay ? customPlay.title : (tacticName || 'Táctica Pizarra');
    const targetCategory = customPlay ? (customPlay.category || 'Ataque') : (tacticCategory || 'Ataque');

    let exportFrames: PlayFrame[] = [];
    if (customPlay && customPlay.frames && customPlay.frames.length > 0) {
      exportFrames = customPlay.frames;
    } else if (!customPlay && frames && frames.length > 0) {
      exportFrames = frames;
    } else {
      exportFrames = [
        {
          id: 'frame-1',
          title: 'Fase 1',
          tokens: customPlay ? customPlay.tokens : [...tokens],
          paths: customPlay ? customPlay.paths : [...paths],
        },
      ];
    }

    const hasContent = exportFrames.some(
      (f) => (f.tokens && f.tokens.length > 0) || (f.paths && f.paths.length > 0)
    );

    if (!hasContent) {
      alert('Coloca jugadores o dibuja trazos tácticos en la pizarra antes de exportar el video.');
      return;
    }

    setIsExportingVideo(true);
    setVideoExportProgress(5);
    setVideoExportStatus('Preparando renderizado de la jugada...');

    try {
      const filename = await downloadTacticalPlayVideo(
        targetPlayTitle,
        targetCategory,
        courtType,
        exportFrames,
      ...
      setTimeout(() => {
        setExportedVideoName(null);
      }, 5000);
    } catch (err: any) {
      console.error('Error exportando video MP4:', err);
      alert('No se pudo generar el video: ' + (err?.message || 'Error en el navegador'));
    } finally {
      ...
      >
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <button
              type="button"
              onClick={() => handleExportMp4Video(play)}
              disabled={isExportingVideo}
              title="Descargar esta táctica en video MP4"
              className="px-2 py-1 rounded-lg bg-purple-600/30 hover:bg-purple-600/50 border border-purple-500/40 text-purple-300 hover:text-white font-extrabold text-[11px] flex items-center gap-1 shadow-sm transition-all active:scale-95 cursor-pointer disabled:opacity-50"
            >
        ...
        <div className="bg-slate-900 border border-slate-700/80 p-4 sm:p-6 rounded-3xl text-white shadow-xl space-y-3">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
              ...
              <button
                onClick={() => handleExportMp4Video(activeViewerPlay)}
                disabled={isExportingVideo}
                className="px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-2xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-extrabold text-xs flex items-center gap-2 shadow-lg shadow-purple-600/30 transition-all active:scale-95 cursor-pointer disabled:opacity-50"
                title="Descargar esta jugada táctica animada en video MP4"
              >
  ...
  return (
    <div className="space-y-6 animate-fadeIn max-w-7xl mx-auto pb-12 whiteboard-view" data-board="true" data-allow-action="true">
      {/* Header */}
        ...
        <div className="lg:col-span-8 bg-slate-900 rounded-3xl p-4 sm:p-6 border border-slate-800 shadow-2xl relative select-none space-y-4">
          ...
          <div className="bg-slate-800 p-4 rounded-2xl border border-slate-700 shadow-xl space-y-3">
            <div className="flex items-center gap-3">
              ...
              {/* Quick Export MP4 button on timeline */}
              <button
                onClick={() => handleExportMp4Video()}
                disabled={isExportingVideo}
                className="px-3 py-2.5 rounded-xl bg-purple-600/20 hover:bg-purple-600/40 text-purple-300 hover:text-white border border-purple-500/40 font-bold text-xs flex items-center gap-1.5 transition-all active:scale-95 cursor-pointer disabled:opacity-50 shrink-0"
                title="Exportar y descargar esta jugada como video MP4"
              >
      ...
      {/* Modal de Progreso de Exportación de Video MP4 */}
      {isExportingVideo && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md animate-fadeIn">
          <div className="bg-slate-900 border border-purple-500/40 rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl space-y-6 text-white text-center relative overflow-hidden">
            {/* Top decorative glow */}
            ...
            <div className="mx-auto w-16 h-16 rounded-2xl bg-gradient-to-tr from-purple-600 to-indigo-600 flex items-center justify-center text-white shadow-xl shadow-purple-600/40 animate-pulse">
              <Film className="w-8 h-8" />
            </div>

            <div className="space-y-2">
              <h3 className="text-xl font-black tracking-tight text-white">
                Generando Video MP4 de la Pizarra
              </h3>
              <p className="text-xs text-slate-400 leading-relaxed">
                Renderizando fotogramas tácticos en alta definición (1280x800) con sincronización de movimientos, pases y cortes.
              </p>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs font-mono font-bold">
                <span className="text-purple-300 flex items-center gap-1.5">
                  ...
                  {videoExportStatus || 'Procesando...'}
                </span>
                <span className="text-amber-300 text-sm">{videoExportProgress}%</span>
              </div>
              <div className="w-full bg-slate-800 rounded-full h-3 p-0.5 border border-slate-700/80 overflow-hidden">
                <div
                  className="bg-gradient-to-r from-purple-500 via-indigo-500 to-amber-400 h-full rounded-full transition-all duration-200 shadow-sm"
                  style={{ width: `${Math.max(4, Math.min(100, videoExportProgress))}%` }}
                />
              </div>
            </div>

            <p className="text-[11px] text-slate-500 italic">
              Por favor espera unos segundos mientras el navegador codifica el archivo MP4...
            </p>
          </div>
        </div>
      )}

      {/* Toast Flotante de Descarga Exitosa */}
      {exportedVideoName && (
        <div className="fixed bottom-6 right-6 z-50 bg-slate-900 border border-emerald-500/50 text-white px-5 py-3.5 rounded-2xl shadow-2xl flex items-center gap-3 animate-fadeIn">
          <div className="w-8 h-8 rounded-xl bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 flex items-center justify-center shrink-0">
            <Check className="w-5 h-5" />
          </div>
          <div>
            <h5 className="font-extrabold text-xs text-emerald-300">¡Video MP4 descargado!</h5>
            <p className="text-[11px] text-slate-300 font-mono">{exportedVideoName}</p>
          </div>
        </div>
      )}
    </div>
