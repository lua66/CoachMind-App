...
export const CreateTrainingView: React.FC<CreateTrainingViewProps> = ({
  ...
  const [currentDrillDiagrams, setCurrentDrillDiagrams] = useState<DrillVariantDiagram[]>([
    {
      id: 'diag-1',
  ...
  const syncActiveBoardToDiagrams = () => {
    ...
    const currentSvg = boardRef.current ? boardRef.current.getSvgDataUrl() : '';

    const updated = [...currentDrillDiagrams];
    if (updated[activeDiagramIndex]) {
      updated[activeDiagramIndex] = {
        ...updated[activeDiagramIndex],
  ...
  const handleSelectDiagram = (targetIdx: number) => {
    ...
    const targetDiag = updated[targetIdx];
    if (targetDiag) {
      setCurrentDrillDiagrams(updated);
  ...
  // Add new variant board to current drill
  const handleAddNewVariant = (duplicateCurrent: boolean = false) => {
    const { updated, currentEls, currentCourt, currentSvg } = syncActiveBoardToDiagrams();
    const nextVariantNumber = updated.length;
    const newDiag: DrillVariantDiagram = {
      id: `diag-${Date.now()}-${nextVariantNumber + 1}`,
      title: `Variante ${nextVariantNumber}: Progresión táctica`,
      courtType: currentCourt,
    ...
    setCurrentDrillDiagrams(newDiagramsList);
  ...
  // Delete a variant board
  const handleDeleteVariant = (dIdx: number) => {
    ...
    const targetDiag = filtered[nextActive];
    if (targetDiag) {
      setCurrentCourtType(targetDiag.courtType || 'half');
      setCurrentDiagramElements(targetDiag.diagramElements || []);
      setBoardResetKey((prev) => prev + 1);
  ...
  // Rename a variant board
  const handleUpdateVariantTitle = (dIdx: number, newTitle: string) => {
    setCurrentDrillDiagrams((prev) => {
      const copy = [...prev];
      if (copy[dIdx]) {
        copy[dIdx] = { ...copy[dIdx], title: newTitle };
      }
      return copy;
    });
  };
  ...
  const handleSaveCurrentDrill = () => {
    ...
    setCurrentDiagramElements([]);
    setCurrentCourtType('half');
    const freshDiag: DrillVariantDiagram = {
      id: `diag-${Date.now()}-1`,
      title: 'Pizarra Principal',
      courtType: 'half',
      diagramElements: [],
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-8 space-y-4">
          ...
          <div className="bg-white p-4 rounded-3xl border border-slate-200/80 shadow-sm space-y-3">
            ...
            {drills.length > 0 && (
              <div className="flex flex-wrap gap-2 pt-1 border-t border-slate-100">
                {drills.map((d, idx) => {
                  const variantCount = d.diagrams && d.diagrams.length > 0 ? d.diagrams.length : 1;
                  return (
                    <div
                      key={d.id || idx}
                    ...
                    >
                      ...
                      {variantCount > 1 && (
                        <span className="text-[9px] px-1.5 py-0.5 rounded-md bg-orange-500/20 text-orange-400 font-extrabold flex items-center gap-0.5">
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm space-y-4">
            ...
            {/* Multi-Board / Variants Control Bar */}
            <div className="p-3.5 rounded-2xl bg-slate-900 text-white border border-slate-800 space-y-3 shadow-inner">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded-xl bg-orange-500/20 text-orange-400 flex items-center justify-center font-black shrink-0 border border-orange-500/30">
                    <Layers className="w-4 h-4" />
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <h4 className="text-xs sm:text-sm font-extrabold text-white">
                        Pizarras y Variantes Tácticas
                      </h4>
                      <span className="text-[10px] bg-orange-500/30 text-orange-300 px-2 py-0.5 rounded-full font-bold border border-orange-500/30">
                        {currentDrillDiagrams.length} {currentDrillDiagrams.length === 1 ? 'pizarra' : 'pizarras'}
                      </span>
                    </div>
                    <p className="text-[10px] sm:text-[11px] text-slate-400">
                      Dibuja la situación base y agrega todas las variantes que tenga este ejercicio.
                    </p>
                  </div>
                </div>

                {/* Actions: Add Variant / Duplicate */}
                <div className="flex items-center gap-2 flex-wrap">
                  <button
                    type="button"
                    onClick={() => handleAddNewVariant(false)}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-orange-500 hover:bg-orange-600 text-slate-950 font-black text-xs shadow-md shadow-orange-500/20 transition-all cursor-pointer hover:scale-[1.02]"
                    title="Añadir una nueva pizarra en blanco para una variante de este ejercicio"
                  >
                    <Plus className="w-3.5 h-3.5" />
                    <span>+ Añadir Pizarra / Variante</span>
                  </button>

                  <button
                    type="button"
                    onClick={() => handleAddNewVariant(true)}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs border border-slate-700 transition-all cursor-pointer"
                    title="Duplica la pizarra actual para dibujar la continuación de la jugada sin empezar de cero"
                  >
                    <Copy className="w-3.5 h-3.5 text-amber-400" />
                    <span>Duplicar para Continuación</span>
                  </button>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 overflow-x-auto pb-1 pt-1 border-t border-slate-800/80">
                {currentDrillDiagrams.map((diag, dIdx) => {
                  const isActive = activeDiagramIndex === dIdx;
                  return (
                    <div
                      key={diag.id || dIdx}
                      onClick={() => handleSelectDiagram(dIdx)}
                      className={`group flex items-center gap-2 px-3.5 py-1.5 rounded-xl text-xs transition-all shrink-0 border cursor-pointer ${
                        isActive
                          ? 'bg-gradient-to-r from-amber-500 to-orange-500 text-slate-950 border-orange-400 font-extrabold shadow-md'
                          : 'bg-slate-800/90 hover:bg-slate-800 text-slate-300 border-slate-700 hover:text-white font-semibold'
                      }`}
                    >
                      <span className="text-sm">{dIdx === 0 ? '📋' : '🔀'}</span>
                      <span className="max-w-[150px] sm:max-w-[200px] truncate">
                        {diag.title || (dIdx === 0 ? 'Pizarra Principal' : `Variante ${dIdx}`)}
                      </span>
                      ...
                      {currentDrillDiagrams.length > 1 && (
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            handleDeleteVariant(dIdx);
                          }}
                          className={`p-1 rounded-lg hover:bg-black/20 text-slate-400 hover:text-red-400 transition-colors ${
                            isActive ? 'text-slate-900 hover:text-red-800' : ''
                          }`}
                          title="Eliminar esta pizarra de variante"
                        >
                          <Trash2 className="w-3 h-3" />
                        </button>
                      )}
              ...
              {/* Active Variant Name Input */}
              {currentDrillDiagrams[activeDiagramIndex] && (
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-2 border-t border-slate-800/60 text-xs">
                  <div className="flex items-center gap-2 flex-1">
                    <span className="text-[11px] font-bold text-slate-400 shrink-0">
                      Nombre de esta pizarra:
                    </span>
                    <input
                      type="text"
                      value={currentDrillDiagrams[activeDiagramIndex]?.title || ''}
                      onChange={(e) => handleUpdateVariantTitle(activeDiagramIndex, e.target.value)}
                      placeholder="Ej: Variante 1: Defensa zonal / 2c1 tras pase exterior"
                      className="px-3 py-1.5 rounded-xl bg-slate-950 border border-slate-700 text-slate-100 font-semibold text-xs focus:outline-none focus:border-orange-500 flex-1 max-w-md"
                    />
                  </div>

                  <span className="text-[11px] text-slate-400">
                    Editando: <strong className="text-orange-400">{currentDrillDiagrams[activeDiagramIndex]?.title}</strong>
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-slate-700 flex items-center justify-between">
                <span>
                  Lienzo Táctico ({currentDrillDiagrams[activeDiagramIndex]?.title || 'Pizarra activa'})
                </span>
                <span className="text-[10px] text-slate-400 font-normal">
                  Haz clic y arrastra para dibujar fichas y trayectorias
                </span>
              </label>

              <TacticalDrillBoard
                key={`board-${boardResetKey}-${activeDrillIndex !== null ? `drill-${activeDrillIndex}` : 'new-drill'}-var-${activeDiagramIndex}`}
                ref={boardRef}
